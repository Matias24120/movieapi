...
const WatchlistMovies = () => {
    ...
    useEffect(() => {
        const fetchWatchlist = async () => {
            try {
                ...
                if (storedWatchlist) {
                    const parsedWatchlist = JSON.parse(storedWatchlist).filter(movieId => movieId !== null);
    ...
    useEffect(() => {
        const fetchMoviesDetails = async () => {
            try {
                const allMoviesDetails = await Promise.all(watchlistMovies.map(async (movieId) => {
                    try {
    ...
    return (
        <div className="text-center">
            <h2 style={{backgroundColor: '#241744', color: 'white', padding: '14px', marginBottom: '20px'}} >Watchlist</h2>
